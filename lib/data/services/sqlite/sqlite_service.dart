...
class SqliteService {
  Future<List<EmployeeDTO>?> getEmployeeList() async {
    ...
    final List<Map<String, Object?>> queryResult = await _db.query('Employees');
  ...
  Future<List<EmployeeDTO>?> getEmployee(int id) async {
    ...
    final List<Map<String, Object?>> queryResult =
        await _db.query('Employee', where: "emp_id = ?", whereArgs: [id]);
    return queryResult.map((e) => EmployeeDTO.fromMap(e)).toList();
  }

  Future<void> updateEmployee(int id, EmployeeDTO employeeDTO) async {
    final Database _db = await SqliteClient().initializeDB();
    await _db.update('Employees', employeeDTO.toMap(),
        where: "emp_id = ?", whereArgs: [id]);
  }

  Future<void> addEmployee(String name, String role, int dateOfJoining) async {
    final Database _db = await SqliteClient().initializeDB();
    var employee =
        EmployeeDTO(name: name, role: role, dateOfJoining: dateOfJoining);
